...
<?php
  ...
  // EVENTS 
  $events = mysql_query("SELECT *, DATE_FORMAT(date, '%a') as day, DATE_FORMAT(date, '%b') as month 
                         FROM `downtoz0_cms`.`events` 
                         WHERE `feature` = '' && `hide` = FALSE && `end_date` >= CURDATE() 
                         ORDER BY `date` ASC LIMIT $start, $numPerPage");
